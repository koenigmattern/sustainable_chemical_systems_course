...
def convert_notebook_to_html(input_path, output_path, curr_filtered_notebooks):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(script_dir, ".."))
    jupyter_cmd = _resolve_jupyter_cmd(project_root)
    ...
    for notebook in curr_filtered_notebooks:
        ...
        output_file = os.path.join(
            output_path, notebook.replace(".ipynb", ".html")
        )
        ...
        try:
            subprocess.run(cmd, check=True)
            print("✔ HTML document created:", output_file)
        except subprocess.CalledProcessError as e:
            print("✘ Conversion failed for:", input_file)
            print("Command:", " ".join(cmd))
            print("Error:", e)
...
html_slides_folder = os.path.join(project_root, "html_export")
...
convert_notebook_to_html(suschem_folder, html_slides_folder, notebooks)
